...
この操作で、グローバルの `uloop` dispatcher と、このプロジェクト用の `.uloop/bin/uloop` CLI bundle がインストールまたは更新されます。
...
このコマンドでインストールされるのはグローバル dispatcher のみです。各 Unity プロジェクトでは Window > Unity CLI Loop > Settings を開き、**Install CLI** を押して `.uloop/bin/uloop` を更新してください。

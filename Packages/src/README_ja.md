...
Settings ウィンドウでは、グローバルな `uloop` ランチャーが検出されているかを確認できます。このプロジェクト用の `.uloop/bin/uloop` CLI bundle は、未作成またはバージョン不一致のときに Unity Package が自動更新します。
...
この legacy npm dispatcher は v3 beta 移行中の互換導線として残っています。Go 製 CLI では、上記の native installer を推奨します。

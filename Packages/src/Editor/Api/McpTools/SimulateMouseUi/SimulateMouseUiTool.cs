    ...
    public class SimulateMouseUiTool : AbstractUnityTool<SimulateMouseUiSchema, SimulateMouseUiResponse>
    {
            ...
            CancellationToken ct)
        {
            ...
            switch (parameters.Action)
            {
                ...
                case MouseAction.Replay:
                    response = await ExecuteReplay(parameters, eventSystem, ct);
                    break;
        ...
        private async Task<SimulateMouseUiResponse> ExecuteReplay(
            SimulateMouseUiSchema parameters, EventSystem eventSystem, CancellationToken ct)
        {
            if (string.IsNullOrEmpty(parameters.RecordingFilePath))
            {
                return new SimulateMouseUiResponse
                {
                    Success = false,
                    Message = "RecordingFilePath is required for Replay action.",
                    Action = MouseAction.Replay.ToString()
                };
            }

            InputRecordingData? data = InputRecordingFileHelper.Load(parameters.RecordingFilePath);
            if (data == null)
            {
                return new SimulateMouseUiResponse
                {
                    Success = false,
                    Message = $"Failed to load recording: {parameters.RecordingFilePath}",
                    Action = MouseAction.Replay.ToString()
                };
            }
            ...
            HashSet<MouseButton> heldButtons = new HashSet<MouseButton>();
            ...
            Vector2 pressScreenPos = Vector2.zero;
            ...
            for (int frame = 0; frame <= totalFrames; frame++)
            {
                ...
                while (eventIndex < data.Frames.Count && data.Frames[eventIndex].Frame <= frame)
                {
                    InputFrameEvents frameEvents = data.Frames[eventIndex];
                    for (int i = 0; i < frameEvents.Events.Count; i++)
                    {
                        RecordedInputEvent evt = frameEvents.Events[i];
                        switch (evt.Type)
                        {
                            case InputEventTypes.KEY_DOWN:
                                if (Enum.TryParse(evt.Data, true, out Key keyDown))
                                {
                                    heldKeys.Add(keyDown);
                                }
                                break;
                            case InputEventTypes.KEY_UP:
                                if (Enum.TryParse(evt.Data, true, out Key keyUp))
                                {
                                    heldKeys.Remove(keyUp);
                                }
                                break;
                            case InputEventTypes.MOUSE_POSITION:
                                mousePosition = InputRecorder.ParseVector2(evt.Data);
                                break;
                            case InputEventTypes.MOUSE_DELTA:
                                mouseDelta = InputRecorder.ParseVector2(evt.Data);
                                break;
                            case InputEventTypes.MOUSE_SCROLL:
                                if (float.TryParse(evt.Data, System.Globalization.NumberStyles.Float,
                                    System.Globalization.CultureInfo.InvariantCulture, out float scrollY))
                                {
                                    mouseScroll = new Vector2(0f, scrollY);
                                }
                                break;
                            case InputEventTypes.MOUSE_CLICK:
                                if (Enum.TryParse(evt.Data, true, out MouseButton clickBtn))
                                {
                                    if (!heldButtons.Contains(clickBtn))
                                    {
                                        buttonJustPressed = clickBtn == MouseButton.Left;
                                    }
                                    heldButtons.Add(clickBtn);
                                }
                                break;
                            case InputEventTypes.MOUSE_RELEASE:
                                if (Enum.TryParse(evt.Data, true, out MouseButton relBtn))
                                {
                                    if (heldButtons.Contains(relBtn))
                                    {
                                        buttonJustReleased = relBtn == MouseButton.Left;
                                    }
                                    heldButtons.Remove(relBtn);
                                }
                                break;
                        }
                    }
                ...
                if (keyboard != null)
                {
                    InputReplayer.ApplyKeyboardSnapshot(keyboard, heldKeys);
                }

                // Inject mouse state (position + delta + scroll + buttons)
                Mouse? mouse = Mouse.current;
                if (mouse != null)
                {
                    InputReplayer.ApplyMouseSnapshot(mouse, heldButtons, mouseDelta, mouseScroll, mousePosition);
                }

                // UI overlay + ExecuteEvents
                if (mousePosition.HasValue)
                {
                    Vector2 screenPos = mousePosition.Value;
                    ...
                    if (buttonJustPressed)
                    {
                        ...
                        if (hit != null)
                        {
                            ...
                            dragTarget = ExecuteEvents.GetEventHandler<IDragHandler>(rawTarget);
                            if (dragTarget != null)
                            {
                                ExecuteEvents.Execute(dragTarget, pointerData, ExecuteEvents.initializePotentialDrag);
                            }
                        ...
                        SimulateMouseUiOverlayState.Update(
                            MouseAction.Click, inputPos, null, pressTarget?.name, gameViewSize);
                        replayedActions++;
                    }
                    else if (heldButtons.Contains(MouseButton.Left) && pointerData != null)
                    {
                        // Drag or long press while held
                        Vector2 delta = screenPos - pointerData.position;
                        ...
                        if (delta != Vector2.zero)
                        {
                            ...
                            {
                                float distance = (screenPos - pressScreenPos).magnitude;
                                if (distance > eventSystem.pixelDragThreshold)
                                {
                                    ...
                                    Vector2 pressInputPos = new Vector2(pressScreenPos.x, gameViewSize.y - pressScreenPos.y);
                                    SimulateMouseUiOverlayState.Update(
                            ...
                            if (isDragging && dragTarget != null)
                            {
                                ...
                                SimulateMouseUiOverlayState.UpdatePosition(inputPos);
                                SimulateMouseUiOverlayState.AddWaypoint(inputPos);
                            }
                        }

                        if (!isDragging)
                        {
                            // Long press visualization
                            float elapsed = frame / 60f; // approximate
                            SimulateMouseUiOverlayState.Update(
                                MouseAction.LongPress, inputPos, null, pressTarget?.name, gameViewSize);
                            SimulateMouseUiOverlayState.UpdateLongPressElapsed(elapsed);
                        }
                    }

                    if (buttonJustReleased && pointerData != null)
                    {
                        // Pointer up
                        if (pressTarget != null)
                        {
                        ...
                        if (isDragging && dragTarget != null)
                        {
                            ...
                            if (dropHit != null)
                            {
                                pointerData.pointerCurrentRaycast = dropHit.Value;
                        ...
                        else if (pressTarget != null)
                        {
                            GameObject? clickTarget = ExecuteEvents.GetEventHandler<IPointerClickHandler>(
                                pointerData.rawPointerPress ?? pressTarget);
                            if (clickTarget != null)
                            {
                                ExecuteEvents.Execute(clickTarget, pointerData, ExecuteEvents.pointerClickHandler);
                            }
                    ...
                    {
                        // Idle: just show cursor position
                        SimulateMouseUiOverlayState.Update(

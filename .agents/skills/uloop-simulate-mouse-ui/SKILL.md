...
| `--bypass-raycast` | boolean | `false` | For `Click`, `Drag`, and `DragStart`, bypass EventSystem raycast and dispatch pointer events directly to `--target-path`. Use when a raycast-blocking overlay visually covers the intended target. |
| `--target-path` | string | `""` | Hierarchy path of the target GameObject, for example `Canvas/Panel/Button`. Required when `--bypass-raycast true` is used with `Click`, `Drag`, or `DragStart`; prefer `AnnotatedElements[].Path` from screenshot JSON. |
...
- `--bypass-raycast true` still uses coordinates for pointer event positions, but chooses the clicked or dragged GameObject by `--target-path`

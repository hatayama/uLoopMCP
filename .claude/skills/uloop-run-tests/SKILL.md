...
description: "Execute Unity Test Runner and get detailed results. Use when you need to: (1) Run EditMode or PlayMode unit tests, (2) Verify code changes pass all tests, (3) Diagnose test failures with error messages and stack traces. Executes via `uloop run-tests` CLI invocation (single-flight only — never run multiple `uloop run-tests` in parallel); auto-saves NUnit XML results to `.uloop/outputs/TestResults/` on failure."
---
